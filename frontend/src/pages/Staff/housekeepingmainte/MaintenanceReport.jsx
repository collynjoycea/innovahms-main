...
const MaintenanceReport = ({ isDarkMode }) => {
  const [activeModal, setActiveModal] = useState(null);

  // OPTIMIZED THEME SYSTEM - Synced with your Dashboard style
  const theme = {
      ...
      <div className={`${theme.card} border w-full max-w-md rounded-[2.5rem] overflow-hidden ${theme.shadow}`}>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="flex gap-4">
            <button onClick={onClose} className={`flex-1 py-4 rounded-xl font-black uppercase text-[10px] tracking-widest border ${theme.border} ${theme.textMain} hover:opacity-70 transition-all`}>
              Dismiss
            </button>
            <button className="flex-1 py-4 rounded-xl font-black uppercase text-[10px] tracking-widest bg-[#c9a84c] text-black shadow-lg shadow-[#c9a84c]/20 hover:scale-[1.02] active:scale-95 transition-all">
              Save Report
            </button>
          </div>
        </div>
  ...
  const recentHistory = [
    { id: '302', issue: 'Broken shower head', status: 'Pending', time: '10 mins ago' },
    { id: '203', issue: 'Torn curtain rail', status: 'In Progress', time: '2 hours ago' },
    { id: '105', issue: 'AC vent cleaning', status: 'Resolved', time: 'Yesterday' }
  ];
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-5 gap-10">
        ...
        <div className="lg:col-span-3">
          <div className={`${theme.card} border rounded-[3rem] p-10 ${theme.shadow} relative overflow-hidden`}>
            {/* Design Element - Golden Glow (Dark Mode only) */}
            {isDarkMode && <div className="absolute top-0 right-0 w-64 h-64 bg-[#c9a84c]/5 blur-[100px] rounded-full"></div>}
            ...
            <form className="space-y-8 text-left relative z-10" onSubmit={(e) => { e.preventDefault(); setActiveModal('success'); }}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" required placeholder="e.g. Room 304" className={`w-full p-4 pl-12 rounded-2xl border ${theme.input} text-[12px] font-bold outline-none focus:ring-2 focus:ring-[#c9a84c]/20 transition-all`} />
                  </div>
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase tracking-widest text-[#c9a84c] ml-1">Severity</label>
                  <select className={`w-full p-4 rounded-2xl border ${theme.input} text-[12px] font-bold outline-none cursor-pointer appearance-none`}>
              ...
              <div className="space-y-3">
                ...
                <textarea required placeholder="Briefly explain the issue..." className={`w-full p-6 rounded-3xl border ${theme.input} h-40 text-[12px] font-medium outline-none resize-none transition-all`} />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <div className="flex gap-4">
                    {["No", "Yes"].map((opt) => (
                      <button type="button" key={opt} className={`flex-1 py-4 rounded-xl border ${theme.border} text-[10px] font-black uppercase tracking-widest transition-all ${opt === "Yes" ? "bg-red-500/10 border-red-500/50 text-red-500" : `${theme.btnSecondary}`}`}>
                        {opt}
                      </button>
                    ))}
                  </div>
              ...
              {recentHistory.map((item, i) => (
                <div key={i} className={`p-6 rounded-[2rem] border ${theme.border} ${theme.innerCard} text-left relative overflow-hidden group transition-all hover:scale-[1.02]`}>
                  <div className="flex justify-between items-start mb-2 relative z-10">
                    <h4 className="text-[15px] font-black uppercase tracking-tight">Room {item.id}</h4>
                    <span className={`text-[8px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full border ${theme.border} ${item.status === 'Pending' ? 'bg-orange-500/10 text-orange-500' : item.status === 'Resolved' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-[#c9a84c]/10 text-[#c9a84c]'}`}>
                      ● {item.status}
                    </span>
                  </div>
