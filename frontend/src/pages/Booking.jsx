...
import { ArrowRight, CheckCircle2, Loader2, MapPin, QrCode, Sparkles, Star, X } from 'lucide-react';
...
export default function Booking() {
  ...
  const total = Number(Math.max(0, baseTotal - discountAmount).toFixed(2));
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div className="space-y-4 lg:col-span-2">
              ...
              <form onSubmit={submit}>
                <div className="mb-4 rounded-[1.5rem] border border-white/15 bg-white/8 p-6 backdrop-blur-xl"><div className="mb-5 flex items-center gap-3"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#bf9b30] text-[11px] font-black text-[#0d0c0a]">1</span><h2 className="text-sm font-black uppercase tracking-widest text-white">Stay Duration & Arrival Time</h2></div><div className="grid grid-cols-2 gap-4"><div><label className="mb-1.5 block text-[9px] font-black uppercase tracking-[0.2em] text-[#bf9b30]">Check-In Date</label><input type="date" value={checkIn} min={today} onChange={(e) => setCheckIn(e.target.value)} className={inputCls} /></div><div><label className="mb-1.5 block text-[9px] font-black uppercase tracking-[0.2em] text-[#bf9b30]">Check-Out Date</label><input type="date" value={checkOut} min={checkIn} onChange={(e) => setCheckOut(e.target.value)} className={inputCls} /></div><div><label className="mb-1.5 block text-[9px] font-black uppercase tracking-[0.2em] text-[#bf9b30]">Expected Arrival Time</label><input type="time" value={checkInTime} onChange={(e) => setCheckInTime(e.target.value)} className={inputCls} /></div><div><label className="mb-1.5 block text-[9px] font-black uppercase tracking-[0.2em] text-[#bf9b30]">Expected Check-Out Time</label><input type="time" value={checkOutTime} onChange={(e) => setCheckOutTime(e.target.value)} className={inputCls} /></div></div></div>
